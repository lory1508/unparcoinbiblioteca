...
function storeSchemasActionForCommand(flags, context) {
...
async function storeSchemasAction(flags, context) {
  const {
    workspaceName,
    verbose,
    idPrefix,
    ...
    extractManifest,
    schemaRequired
  } = schemaApiClient.parseStoreSchemasConfig(flags, context), {
    output,
    apiClient,
    jsonReader,
    manifestExtractor
  ...
  try {
    ...
    if (failures.length)
      throw new Error(`Failed to store ${failures.length}/${targetWorkspaces.length} schemas. Successfully stored ${successes.length}/${targetWorkspaces.length} schemas.`);
    return output.success(`Stored ${successes.length}/${targetWorkspaces.length} schemas`), "success";
  } catch (err) {
...
function createStoreWorkspaceSchema(args) {
  ...
  return async (workspace) => {
    ...
    try {
      ...
      throw output.error(`\u21B3 Error storing schema for workspace "${workspace.name}":
  ${chalk__default.default.red(`${err.message}`)}`), err;
    }
  };
}
exports.default = storeSchemasActionForCommand;
