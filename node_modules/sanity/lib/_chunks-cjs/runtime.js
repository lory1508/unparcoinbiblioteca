...
async function renderDocumentFromWorkerData() {
  ...
  const {
    ...
    jsx: "automatic",
    loader: "jsx"
  }), html = await getDocumentHtml(studioRootPath, props, importMap, isApp);
  node_worker_threads.parentPort.postMessage({
    type: "result",
    html
  }), unregister(), unregisterJs();
}
async function getDocumentHtml(studioRootPath, props, importMap, isApp) {
  const Document = await getDocumentComponent(studioRootPath, isApp), css = props?.css?.map((url) => {
    try {
...
async function getDocumentComponent(studioRootPath, isApp) {
  debug$2("Loading default document component from `sanity` module");
  const {
    BasicDocument
  } = await import("sanity"), {
    DefaultDocument
  } = await import("sanity"), Document = isApp ? BasicDocument : DefaultDocument;
