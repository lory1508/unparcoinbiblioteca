...
async function undeployAppAction(_, context) {
  ...
  const appId = cliConfig && "__experimental_appConfiguration" in cliConfig ? cliConfig.__experimental_appConfiguration?.appId : void 0;
  if (!appId) {
    spinner.fail(), output.print("No application ID provided."), output.print("Please set appId in `__experimental_appConfiguration` in sanity.cli.js or sanity.cli.ts."), output.print("Nothing to undeploy.");
    return;
  }
