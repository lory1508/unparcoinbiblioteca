...
async function startAppDevServer(args, context) {
  ...
  let organizationId;
  cliConfig && "__experimental_appConfiguration" in cliConfig && cliConfig.__experimental_appConfiguration?.organizationId && (organizationId = cliConfig.__experimental_appConfiguration.organizationId), organizationId || (output.error("Apps require an organization ID (orgId) specified in your sanity.cli.ts file"), process.exit(1));
  ...
  try {
    ...
    await devAction.startDevServer({
      ...
      skipStartLog: !0,
      isApp: !0
    }), spinner.succeed(), output.print(`Dev server started on port ${config.httpPort}`), output.print("View your app in the Sanity dashboard here:"), output.print(chalk__default.default.blue(chalk__default.default.underline(devAction.getCoreAppURL({
      organizationId,
