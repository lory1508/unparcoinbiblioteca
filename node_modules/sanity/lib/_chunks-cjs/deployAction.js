...
async function deployAppAction(args, context) {
  const {
    ...
    flags,
    cliConfig
  }), installedSanityVersion = await helpers.getInstalledSanityVersion(), appId = cliConfig && "__experimental_appConfiguration" in cliConfig && cliConfig.__experimental_appConfiguration?.appId, client = apiClient({
    requireUser: !0,
    requireProject: !1
...
Add ${chalk.cyan(`appId: '${userApplication.id}'`)}`), output.print("to __experimental_appConfiguration in sanity.cli.js or sanity.cli.ts"), output.print("to avoid prompting on next deploy."));
  } catch (err) {
