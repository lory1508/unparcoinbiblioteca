...
async function getOrCreateApplication({
  ...
  const {
    prompt,
    cliConfig
  } = context, organizationId = cliConfig && "__experimental_appConfiguration" in cliConfig && cliConfig.__experimental_appConfiguration?.organizationId;
  spinner.succeed();
  const selectedApp = await selectExistingApplication({
    client,
    prompt,
...
async function getOrCreateAppFromConfig({
  ...
  return output.print("The appId provided in your configuration is not recognized."), output.print("Checking existing applications..."), getOrCreateApplication({
    client,
    context,
    spinner
  });
